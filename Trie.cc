...
void Trie::findWords(Board& sopa, int i, int j, set<pair<pair<int, int>, string>>& visited, string s) {
    int n = sopa.getSize();
    if (i < 0 or j < 0 or i >= n or j >= n or visited.find(make_pair(make_pair(i, j), s)) != visited.end() or children[sopa.getCasella(i, j) - 'A'] == NULL) return;

    s.push_back(sopa.getCasella(i, j));

    if (children[sopa.getCasella(i, j) - 'A']->is_word) cout << "Found " << s << endl;
    visited.insert(make_pair(make_pair(i, j), s));
